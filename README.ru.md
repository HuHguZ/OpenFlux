...
- Документ считается рабочим, пока по нему приходят keepalive пира. Документ,
  который подключён, но до пира больше не доходит (пир вышел или попал на
  другой backend документа), перестаёт получать трафик через 25 с, отключённый -
  сразу. Его соединения переезжают на другой документ и возвращаются, когда он
  восстановится.
